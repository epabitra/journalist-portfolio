  ...
  const loadData = async () => {
    try {
      ...
      if (postsData?.success && postsData.data?.length > 0) {
        ...
        let sortedPosts = [...postsData.data];
  ...
  return (
    <>
      <EnhancedHelmet
        ...
        type="website"
        canonicalUrl={`${ENV.SITE_URL || 'https://www.sugyansagar.com'}/blog`}
      />
            ...
            {displayCategories.length > 0 && (
              <div className="category-filter" style={{ marginBottom: 'var(--space-8)' }}>
                ...
                {displayCategories.map((category) => {
                  ...
                  const isActive = selectedCategory.toLowerCase() === categoryValue.toLowerCase();
                  return (
                    <button
                    ...
                    >
                      {category.icon ? (
                        <span style={{ marginRight: '6px', fontSize: '1.1em' }}>{category.icon}</span>
                      ) : null}
                      {category.name}
                    </button>
                  );
                    ...
                    <article key={post.id} className="post-card">
                      ...
                      <div className="post-content mt-4">
                        <div className="post-meta">
                          ...
                          {post.category && (() => {
                            const categoryObj = displayCategories.find(
                              cat => (cat.name || '').toLowerCase() === (post.category || '').toLowerCase() ||
                                     (cat.slug || '').toLowerCase() === (post.category || '').toLowerCase()
                            );
                            return (
                              <span 
                              ...
                              >
                                {categoryObj?.icon && (
                                  <span style={{ marginRight: '4px' }}>{categoryObj.icon}</span>
                                )}
                                {post.category}
                              </span>
                            );
